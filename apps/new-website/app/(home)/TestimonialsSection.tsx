...
const showcaseItems = [
  {
    ...
    playstore: "https://play.google.com/store/apps/details?id=com.noona.app",
  },
];
  ...
  return (
    <>
      <h1 id="showcase" className="text-3xl font-bold pb-8 border-t border-dashed backdrop-blur w-full dark:opacity-90 -mt-[1px] pt-9">
        <div className="w-fd-container max-md:px-4 mx-auto text-left">Who is using NativeWind?</div></h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 flex-wrap text-left gap-8 justify-around p-8 [mask:linear-gradient(to_right,transparent,red_1rem,red_calc(100%-1rem),transparent)] max-w-full mt-2">
          {showcaseItems.map((item) => (
            <div key={item.name} className="group rounded-2xl border border-dashed p-4 flex-1 bg-fd-card relative mt-24">
              ...
              <Image src={item.logo} className="rounded-2xl h-16 w-16 -mt-12 mb-4 backdrop-blur left-4 border border-dashed bg-fd-muted object-cover" alt={`${item.name} logo`} />
              <b className="font-bold text-xl">{item.name}</b>
              <p className="opacity-50 mt-2 text-pretty">{item.description}</p>
            </div>
          ))}
        </div>
      </section>
